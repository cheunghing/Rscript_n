...
handle <-
  getCurlHandle(
    httpheader = list(
      ...
      Cookie = 'JSESSIONID=1B8BEDA240AFCA573BD035ECF4907EC7; theme=theme_base; userName=%E6%9D%8E%E9%95%BF%E5%85%B4; token=a25b085949531e494c422dccc17638b6; userId=s00580; userType=CBUSER',
...
getdata <- function(start_num, length, uri, handle) {
  form_credit_report <- c(
    ...
    'custNo' = '',
    'pbcApplyType' = '',
    'startCreateDatetime' = '2018-09-30 00:00:00',
    'endCreateDatetime' = '2018-09-30 23:59:59',
...
s_1 <- subset(status, status$状态 == '10')
s_2 <- subset(status, status$状态 == '90')
status <- merge(s_1, s_2, by = c('渠道'))[, c(1, 3, 5)]
names(status) <- c('渠道', '未结案', '成功')
status$总数 <- status$未结案+status$成功


report_10 <- subset(credit_report, credit_report$status == '10')
status_10 <-
